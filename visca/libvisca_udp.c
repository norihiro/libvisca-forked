...
{
	debug_udp("udp: send header=%s payload length=%d buf=%s", b2s(ctx->buf_send, 8), ctx->buf_send_n-8, b2s(ctx->buf_send+8, ctx->buf_send_n-8));
	int ret = sendto(ctx->sockfd, ctx->buf_send, ctx->buf_send_n, 0, (struct sockaddr *)&ctx->addr, sizeof(ctx->addr));
	if (ret >= 8)
		return ret-8;
	else
		return -1;
...
inline static int visca_udp_recv_packet_buf(VISCA_udp_ctx_t *ctx)
{
	...
	if (length>=0) {
		...
		debug_udp("udp: recv header=%s payload length=%d buf=%s", b2s(ctx->buf_recv, 8), length-8, b2s(ctx->buf_recv+8, length-8));
	}
	return length;
}
